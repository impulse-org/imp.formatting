 ...
 */
public class FormattingSpecificationParseController extends
		SimpleLPGParseController implements IParseController {
	private FormattingSpecificationParser parser;

	private FormattingSpecificationLexer lexer;
	...
	public IASTNodeLocator getNodeLocator() {
		return new FormattingSpecificationASTNodeLocator();
	} //return new AstLocator(); }

	public FormattingSpecificationParseController() {
	}

	private void createLexerAndParser(IPath filePath) {
		try {
			lexer = new FormattingSpecificationLexer(filePath.toOSString()); // Create the lexer
			parser = new FormattingSpecificationParser(lexer.getLexStream() /*, project*/); // Create the parser
		} catch (IOException e) {
			throw new Error(e);
		}
	}
	...
	public Object parse(String contents, boolean scanOnly,
		...
		//        	System.err.println("FormattingSpecificationParseController.parse:  caught CoreException while deleting problem markers; continuing to parse regardless");
		//        }
